...
<head>
    ...
    <title>Danh sách hội viên - Hệ thống Gym</title>
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                <form method="GET" action="${pageContext.request.contextPath}/members" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-8">
                        <label class="form-label fw-semibold">Tìm kiếm hội viên</label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="fas fa-search"></i></span>
                            <input type="text" name="keyword" class="form-control" placeholder="Tìm theo tên, email hoặc số điện thoại..." value="${param.keyword}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <button type="submit" class="btn btn-primary me-2"><i class="fas fa-search me-1"></i>Tìm kiếm</button>
                        <a href="${pageContext.request.contextPath}/members" class="btn btn-outline-secondary"><i class="fas fa-redo me-1"></i>Làm mới</a>
                    </div>
                </form>
            </div>
        </div>
                        ...
                        <tbody>
                            <c:forEach var="member" items="${members}" varStatus="loop">
                                <tr>
                                    ...
                                    <td>
                                        <a href="${pageContext.request.contextPath}/members?action=edit&id=${member.memberId}" class="btn btn-sm btn-outline-primary me-1" title="Sửa">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="${pageContext.request.contextPath}/members?action=delete&id=${member.memberId}" class="btn btn-sm btn-outline-danger me-1" title="Xóa" onclick="return confirm('Bạn có chắc chắn muốn xóa hội viên này?');">
                                            <i class="fas fa-trash"></i>
                                        </a>
                                        <a href="${pageContext.request.contextPath}/members?action=history&id=${member.memberId}" class="btn btn-sm btn-outline-info" title="Lịch sử">
                            ...
                            <c:if test="${empty members}">
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-4">
                                        <i class="fas fa-inbox fa-2x mb-2 d-block"></i>Không tìm thấy hội viên nào.
                                    </td>
                                </tr>
                            </c:if>
                        </tbody>
